...
// Возвращает { followers, mediaCount }
...
// Возвращает массив последних N публикаций с их метриками.
export async function fetchInstagramPostMetrics(count = 30) {
  ...
  return items.map((item) => ({
    ...
    reposts: 0, // Instagram не предоставляет число репостов через публичный API
    views: 0,   // просмотры доступны только через отдельный insights-запрос с доп. правами
  }));
}
